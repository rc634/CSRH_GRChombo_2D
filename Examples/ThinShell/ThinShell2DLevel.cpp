...
void ThinShell2DLevel::specificPostTimeStep()
{
    ...
    BoxLoops::loop(WeylOmScalar(m_p.extraction_params.center, m_dx),
                           ...
                           EXCLUDE_GHOST_CELLS);
    // BoxLoops::loop(EffectivePotential(m_p.extraction_params.center, m_dx),
    //                        m_state_new, m_state_diagnostics,
    //                        EXCLUDE_GHOST_CELLS);
    ...
    // Refresh the interpolator and do the interpolation
    // m_gr_amr.m_interpolator->refresh();
    // EffectivePotentialExtraction eff_potential(m_p.extraction_params, m_dt, m_time,
    //                                      first_step, m_restart_time);
    // eff_potential.run(m_gr_amr.m_interpolator);
	...
	double L1_Ham = amr_reductions.norm(c_Ham, 1, true);
        double L1_Mom = amr_reductions.norm(Interval(c_Mom1, c_Mom2), 1, true);
        double L2_Theta = amr_reductions_evolution.norm(c_Theta, 2, true);
	double L1_Theta = amr_reductions_evolution.norm(c_Theta, 1, true);
	SmallDataIO constraints_file("constraint_norms",
        ...
        if (first_step)
        {
            constraints_file.write_header_line({"L^2_Ham", "L^2_Mom", "L1_Ham", "L1_Mom", "L2_Theta", "L1_Theta"});
        }
        constraints_file.write_time_data_line({L2_Ham, L2_Mom, L1_Ham, L1_Mom, L2_Theta, L1_Theta});
