...
template <class data_t> void BosonStar::compute_BH(Cell<data_t> current_cell) const
{
    ...
    bh1.is_BH = (m_params_BosonStar.binary_type[0] == 1);
...
template <class data_t> void BosonStar::compute_BS_binary(Cell<data_t> current_cell) const
{
    ...
    vars.chi = pow(pow(bs1.chi,-4)+pow(bs2.chi,-4),-1./4.);
    // pre-collapsed lapse
    vars.lapse = sqrt(vars.chi);
    ...
    // METRIC 
    FOR2(i,j) {
      vars.h[i][j] = bs1.h[i][j] + bs2.h[i][j] - kroneker_helfer[i][j];
      gamma[i][j] = vars.h[i][j] / vars.chi;
    }
    // cartoon term 
    vars.hww = bs1.h[2][2] + bs2.h[2][2] - kroneker_helfer[2][2];

    // CURVATURE
    vars.K = bs1.trK + bs2.trK;
    // superimpose curvatures and curvatures
    for (size_t i = 0; i < 3; i++) {
      for (size_t j = 0; j < 3; j++) {
        for (size_t k = 0; k < 3; k++) {
            Kij[i][j] += (bs1.K_mixed[k][j]+bs2.K_mixed[k][j]) * gamma[k][i];
        }
      }
    }
    FOR2(i,j) {
...
// // Compute the value of the initial vars on the grid
// template <class data_t> void BosonStar::compute_BS_binary_OLD(Cell<data_t> current_cell) const
// {
//     // Lets goooo
